...
export default function Checkbox({ label, id, checked, handleCheck }: props) {
    ...
    return  <label htmlFor={id} className="transition duration-100 ease-in cursor-pointer flex text-left body-m group hover:bg-lightPurple space-x-4 rounded bg-lightBg dark:bg-darkBg p-3">
            <div className="grid items-center justify-center">
                    <input onChange={(e)=>handleCheck(e)} checked={checked} type="checkbox" id={id} className="peer ring-transparent ring-1 row-start-1 col-start-1 appearance-none checked:bg-primaryPurple checked:ring-0 focus:border-none h-4 w-4 rounded-sm border border-[#828FA33F] outline-none" />
            ...
            <p className={`transition-[text-decoration] ease-in grow  dark:text-white ${checked ? 'line-through text-black/50 dark:text-white/50':''}`}>{label}</p>
        </label>
}